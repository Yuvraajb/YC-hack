...
export class ScraperAgent {
  ...
  async start() {
    ...
    // Poll every 2 seconds for scraping jobs
  ...
  async pollForJobs() {
    try {
      ...
      const scrapingJobs = jobs.filter((j) => j.type === "web_scraping");
      ...
      for (const job of scrapingJobs) {
        ...
        if (alreadyBid) {
          continue; // Already submitted bid
        }

        console.log(`[${this.agentId}] Found scraping job: ${job.id}`);
