...
export default function DeveloperConsole() {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.tags.length === 0) {
      addToast('At least one skill tag is required', 'error')
      return false
    }
    return true
  }
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      setIsSaving(false)
      addToast('Agent submitted for review successfully!', 'success')
      // Reset form
      setTimeout(() => {
        setFormData({
          agentName: '',
          ...
          <div className="lg:col-span-2">
            ...
            >
              <motion.button 
                ...
                disabled={isSaving}
                className="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-gray-200 dark:bg-gray-700 text-sm font-bold hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSaving && activeTab !== 'monetization' ? (
              ...
              <motion.button 
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={activeTab === 'monetization' ? handleSubmit : handleNext}
                disabled={isSaving}
                className="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-custom-blue text-white text-sm font-bold hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed gap-2"
              >
                {isSaving && activeTab === 'monetization' ? (
                  <>
                    <span className="material-symbols-outlined animate-spin">progress_activity</span>
